...
pub fn render_ui(
    ...
    if let Err(e) = status_result {
        error!(
            "Status bar rendering panicked: {:?}. Egui context may not be ready.",
            e
        );
        return;
    }
    ...
    if let Err(e) = help_result {
        error!(
            "Help panel rendering panicked: {:?}. Egui context may not be ready.",
            e
        );
    }
}
...
impl Plugin for UiPlugin {
    fn build(&self, app: &mut App) {
        app.init_resource::<UiFrameCounter>()
