    ...
    for x in 0..size {
        for y in 0..size {
            for z in 0..size {
                ...
                if dist <= radius {
                    model.add_voxel(Voxel { x, y, z, color_index: 0 });
                }
            }
        }
    }
        ...
        for y in 0..size {
            for z in 0..size {
                ...
                if noise > threshold {
                    ...
                    model.add_voxel(Voxel { x, y, z, color_index: color_idx });
                }
            }
        }
    }
...
pub fn generate_warrior(size: u8, body_type: BodyType, seed: &str) -> VoxelModel {
    let base_size = (size as f32 * 0.5).max(16.0) as u8;
    ...
    for y in head_y_start..(head_y_start + head_width) {
        for dx in 0..head_width {
            for dz in 0..head_width {
                ...
                if x < size && z < size {
                    model.add_voxel(Voxel { x, y, z, color_index: 1 }); // Helmet
                }
            }
        }
    }
    ...
    for y in torso_y_start..head_y_start {
        for dx in 0..torso_width {
            for dz in 0..(torso_width - 1) {
                ...
                if x < size && z < size {
                    model.add_voxel(Voxel { x, y, z, color_index: 1 });
                }
            }
        }
    }
...
pub fn generate_mage(size: u8, body_type: BodyType, seed: &str) -> VoxelModel {
        ...
        for dx in 0..hat_size {
            for dz in 0..hat_size {
                ...
                if x < size && z < size && y < size {
                    model.add_voxel(Voxel { x, y, z, color_index: 2 });
                }
            }
        }
    }
    ...
    if orb_z < size {
        model.add_voxel(Voxel { x: center_x, y: orb_y, z: orb_z, color_index: 3 });
    }
...
pub fn generate_knight(size: u8, body_type: BodyType, seed: &str) -> VoxelModel {
    ...
    for dy in 0..2 {
        if plume_y + dy < size {
            model.add_voxel(Voxel { x: center_x, y: plume_y + dy, z: center_z, color_index: 1 });
        }
    }
        ...
        for dx in 0..torso_width {
            for dz in 0..(torso_width - 1) {
                ...
                if x < size && z < size {
                    ...
                    let color = if (x + y + z) % 3 == 0 { 2 } else { 0 };
                    model.add_voxel(Voxel { x, y, z, color_index: color });
                }
            }
        }
    }
...
pub fn generate_archer(size: u8, body_type: BodyType, seed: &str) -> VoxelModel {
        ...
        for dx in 0..head_width {
            for dz in 0..head_width {
                ...
                if x < size && z < size {
                    ...
                    let is_edge = dx == 0 || dx == head_width - 1 || dz == 0 || dz == head_width - 1;
    ...
    for y in (head_y_start - 4)..head_y_start {
        if quiver_x < size {
            model.add_voxel(Voxel { x: quiver_x, y, z: quiver_z, color_index: 2 });
        }
    }
...
pub fn generate_robot(size: u8, body_type: BodyType, seed: &str) -> VoxelModel {
    ...
    if eye_z < size {
        model.add_voxel(Voxel { x: center_x - 1, y: eye_y, z: eye_z, color_index: 2 });
        model.add_voxel(Voxel { x: center_x + 1, y: eye_y, z: eye_z, color_index: 2 });
    }
    ...
    for dy in 0..2 {
        if antenna_y + dy < size {
            model.add_voxel(Voxel { x: center_x, y: antenna_y + dy, z: center_z, color_index: 1 });
        }
    }
    ...
    let core_z = center_z;
    model.add_voxel(Voxel { x: center_x, y: core_y, z: core_z, color_index: 2 });
...
pub fn generate_cat(size: u8, body_type: BodyType, seed: &str) -> VoxelModel {
    ...
    for dy in 0..2 {
        model.add_voxel(Voxel { x: center_x - 1, y: ear_y + dy, z: center_z, color_index: 0 });
        model.add_voxel(Voxel { x: center_x + 1, y: ear_y + dy, z: center_z, color_index: 0 });
    }
...
pub fn generate_dog(size: u8, body_type: BodyType, seed: &str) -> VoxelModel {
    ...
    for dy in 0..2 {
        for dz in 0..2 {
            if snout_z + dz < size {
                model.add_voxel(Voxel { x: center_x, y: head_y + dy, z: snout_z + dz, color_index: 0 });
            }
        }
    }
    ...
    // Nose
    if snout_z + 2 < size {
        model.add_voxel(Voxel { x: center_x, y: head_y, z: snout_z + 2, color_index: 1 });
    }

    // Ears (floppy)
    let ear_y = head_y + head_size - 1;
    for dy in 0..3 {
        model.add_voxel(Voxel { x: center_x - 2, y: ear_y - dy, z: center_z, color_index: 0 });
        model.add_voxel(Voxel { x: center_x + 2, y: ear_y - dy, z: center_z, color_index: 0 });
    }
    ...
    for dy in 0..3 {
        if tail_z > 0 {
            model.add_voxel(Voxel { x: center_x, y: body_y + dy, z: tail_z - 1, color_index: 0 });
        }
    }
...
pub fn generate_bird(size: u8, body_type: BodyType, seed: &str) -> VoxelModel {
    ...
    if beak_z < size {
        model.add_voxel(Voxel { x: center_x, y: head_y, z: beak_z, color_index: 2 });
    }
    ...
    for dx in 1..4 {
        ...
        model.add_voxel(Voxel { x: left_x, y: wing_y, z: center_z, color_index: 1 });

        // Right wing
        let right_x = center_x + body_size / 2 + dx;
        if right_x < size {
            model.add_voxel(Voxel { x: right_x, y: wing_y, z: center_z, color_index: 1 });
        }
    }
...
pub fn generate_fish(size: u8, body_type: BodyType, seed: &str) -> VoxelModel {
    ...
    for dx in 0..3 {
        for dy in 0..3 {
            ...
            let y = body_y - 1 + dy;
            if tail_z > 0 && x < size {
                model.add_voxel(Voxel { x, y, z: tail_z - 1, color_index: 1 });
            }
        }
    }
    ...
    for dz in 1..3 {
        if fin_y < size {
            model.add_voxel(Voxel { x: center_x, y: fin_y, z: center_z + dz, color_index: 1 });
        }
    }
...
pub fn generate_dragon(size: u8, body_type: BodyType, seed: &str) -> VoxelModel {
    ...
    for dy in 0..3 {
        if horn_y + dy < size {
            model.add_voxel(Voxel { x: center_x - 1, y: horn_y + dy, z: center_z, color_index: 3 });
            model.add_voxel(Voxel { x: center_x + 1, y: horn_y + dy, z: center_z, color_index: 3 });
        }
    }
    ...
    // Fire breath
    if snout_z + 3 < size {
        model.add_voxel(Voxel { x: center_x, y: head_y, z: snout_z + 3, color_index: 2 });
    }
    ...
    for dx in 1..4 {
        for dy in 0..3 {
            ...
            let y = wing_y + dy;
            if y < size {
                model.add_voxel(Voxel { x: left_x, y, z: center_z, color_index: 1 });
            }

            // Right wing
            let right_x = center_x + body_width / 2 + dx;
            if right_x < size && y < size {
                model.add_voxel(Voxel { x: right_x, y, z: center_z, color_index: 1 });
            }
        }
    }
...
pub fn generate_bear(size: u8, body_type: BodyType, seed: &str) -> VoxelModel {
    ...
    for dy in 0..2 {
        if ear_y + dy < size {
            // Left ear
            model.add_voxel(Voxel { x: center_x - 2, y: ear_y + dy, z: center_z, color_index: 0 });
            model.add_voxel(Voxel { x: center_x - 2, y: ear_y + dy, z: center_z - 1, color_index: 0 });
            // Right ear
            model.add_voxel(Voxel { x: center_x + 2, y: ear_y + dy, z: center_z, color_index: 0 });
            model.add_voxel(Voxel { x: center_x + 2, y: ear_y + dy, z: center_z - 1, color_index: 0 });
        }
