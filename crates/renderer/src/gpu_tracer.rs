//! GPU-based octree raytracer implementation
//!
//! This module provides high-performance GPU raytracing for octree voxel data
//! using OpenGL shaders. The implementation mirrors the CPU raytracer algorithm
//! but executes in parallel on the GPU for significant performance gains.
//!
//! # Architecture
//!
//! - **Shader-based Traversal**: Octree traversal is performed entirely in GLSL
//! - **3D Texture Storage**: Voxel data is uploaded as a 3D texture to GPU memory
//! - **DDA Stepping**: Uses the same DDA algorithm as the CPU implementation
//! - **Lighting**: Matches CPU tracer lighting model for consistency
//!
//! # Usage
//!
//! ```no_run
...
//! let mut tracer = GpuTracer::new(cube);
//!
//! // Initialize GL resources (requires active GL context)
//! // unsafe { tracer.init_gl(&gl).unwrap(); }
//!
//! // Render
//! // unsafe { tracer.render_to_gl(&gl, 800, 600, 0.0); }
//! ```
...
/// GPU raytracer implementation
/// Uses OpenGL shaders to perform octree raytracing on the GPU
pub struct GpuTracer {
    cube: Rc<Cube<i32>>,
    bounds: CubeBounds,
    // GL resources (Option for cases where GL context isn't available)
    gl_program: Option<GpuTracerGl>,
}

/// GPU-specific OpenGL resources
pub struct GpuTracerGl {
    program: Program,
    vao: VertexArray,
...
impl GpuTracer {
    ...
    pub unsafe fn init_gl(&mut self, gl: &Context) -> Result<(), String> {
        unsafe {
            let gl_program = GpuTracerGl::new(gl, &self.cube)?;
